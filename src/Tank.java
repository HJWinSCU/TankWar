...
public class Tank {
	...
	private Direction Kdirection = Direction.UM; // 初始化方向为向上
	TankClient tc;
	public boolean isInvincible = false;
	public int InvincibleCount = 0;
	private boolean good,Sflag;
	private int x, y;
	private int oldX, oldY;
	private boolean live = true; // 初始化为活着
	private int life = 200; // 初始生命值
	private int StrongBulletsNum=0;//初始超级子弹
	...
	private static Image[] RedtankImags = null;
	static {
		tankImags = new Image[] {
				...
				tk.getImage(BombTank.class.getResource("Images/tankDMe.gif")), 
				tk.getImage(BombTank.class.getResource("Images/tankUMe.gif")),
				tk.getImage(BombTank.class.getResource("Images/tankLMe.gif")),
				tk.getImage(BombTank.class.getResource("Images/tankRMe.gif"))};

	}
	static {
		RedtankImags = new Image[] {
				tk.getImage(BombTank.class.getResource("Images/tankDMe.gif")),
				tk.getImage(BombTank.class.getResource("Images/tankUMe.gif")),
				tk.getImage(BombTank.class.getResource("Images/tankLMe.gif")),
				tk.getImage(BombTank.class.getResource("Images/tankRMe.gif")), };
	...
	//	if (good)
	//		new DrawInvinciblebBar().draw(g); // 创造一个血包
	...
	void move() {
		...
		if (!good) {
			...
			if (step == 0) {                  
				...
				int rn = r.nextInt(directons.length-4);
	...
	void decideDirection() {
		if (!bL && !bU && bR && !bD)  //向右移动
			...
			direction = Direction.LM;

		else if (!bL && bU && !bR && !bD)  //向上移动
			direction = Direction.UM;

		else if (!bL && !bU && !bR && bD) //向下移动
			direction = Direction.DM;
	...
	public StrongBullets Strongfire(){
		if(!live)
			return null;
		int x = this.x + Tank.width / 2 - StrongBullets.width / 2;  //开火位置
		int y = this.y + Tank.length / 2 - StrongBullets.length / 2;
		StrongBullets m = new StrongBullets(x, y + 2, good, Kdirection, this.tc);  //没有给定方向时，向原来的方向发火
		tc.strongBullets.add(m);
		StrongBulletsNum--;
		return m;
	}
	...
	private class DrawInvinciblebBar {
		public void draw(Graphics g) {
			Color c = g.getColor();
	...
	public boolean eatStrong(GetStrong s) {//修改 4
		if (this.live && s.isLive() && this.getRect().intersects(s.getRect())) {
			this.StrongBulletsNum += 10;
			s.setLive(false);
			return true;
		}
		return false;
	...
	public boolean invincible(Invincible b) {
		if (this.live && b.isLive() && this.getRect().intersects(b.getRect())) {
			this.isInvincible = true;      //无敌5秒
