...
public class TankClient extends Frame implements ActionListener {
	...
	GetStrong strong=new GetStrong();
	MenuBar jmb = null;
	...
	List<StrongBullets> strongBullets=new ArrayList<StrongBullets>();
	...
	public void framPaint(Graphics g) {
		...
		home.draw(g); // 画出home
		if(homeTank.InvincibleCount!=0)
			homeTank.InvincibleCount--;
		if(homeTank.InvincibleCount == 0)
			homeTank.isInvincible = false;
		if(!homeTank.isInvincible) {// 画出自己家的坦克
		homeTank.draw(g);
		}
		else {
			if(homeTank.InvincibleCount % 6 < 3)
				homeTank.draw(g);
			else
				homeTank.drawRed(g);
		}
		homeTank.eat(blood);// 充满血--生命值
		homeTank.invincible(invincible);
		homeTank.eatStrong(strong);
		...
		for(int i=0;i<strongBullets.size();i++){//修改   4
			StrongBullets m=strongBullets.get(i);
			m.hitTanks(tanks);
			m.hitTank(homeTank);
			m.hitHome();
	...
	public TankClient() {
		...
		for (int i = 0; i < 32; i++) {
			if (i < 16) {
				...
				otherWall.add(new CommonWall(140 + 20 * i, 168, this));
				...
				otherWall.add(new CommonWall(140 + 20 * (i - 16), 188, this));
				otherWall.add(new CommonWall(220, 400 + 20 * (i - 16), this));
				otherWall.add(new CommonWall(520, 400 + 20 * (i - 16), this));
			}
		}

		for (int i = 0; i < 25; i++) { // 金属墙布局
			if (i < 10) {
				metalWall.add(new MetalWall(205 + 30 * i, 130, this));
				
			} else if (i < 20)
				metalWall.add(new MetalWall(362 + 30 * (i - 10), 210, this));
			else
				metalWall.add(new MetalWall(625, 440 + 30 * (i-20), this));
		}

		for (int i = 0; i < 4; i++) { // 树的布局
			if (i < 4) {
				trees.add(new Tree(0 + 30 * i, 360, this));
				trees.add(new Tree(200 + 60 * i, 360, this));
				trees.add(new Tree(440 + 60 * i, 360, this));
				trees.add(new Tree(670 + 30 * i, 210, this));
				trees.add(new Tree(110 + 20 * i, 130, this));
			}

		}

		theRiver.add(new River(73, 164, this));
		...
		this
				.setTitle("坦克大战——(重新开始：R键  开火：F键)                 姓名：高胜立            班级：09级网络2班          学号：3109006516");
